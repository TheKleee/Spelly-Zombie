...
namespace SpellyZombie
{
    ...
    /// own save. A summon is a spell too: the runes that raise it and the
    /// creature it raises, which the Creature Creator designs.
    public class SpellCreator : EditorWindow
    {
        ...
        void OnEnable()
        {
            _book = CreatureCreator.SharedBook(false);
            if (_picked < 0 || _picked >= _book.spells.Count) _picked = _book.spells.Count > 0 ? 0 : -1;
        ...
        void OnGUI()
        {
            ...
            if (_book != CreatureCreator.SharedBook(false)) { _book = CreatureCreator.SharedBook(false); _shownFor = null; }
        ...
        void DrawList()
        {
            using (new EditorGUILayout.VerticalScope(GUILayout.Width(160)))
            {
                ...
                for (int i = 0; i < _book.spells.Count; i++)
                {
                    ...
                    string tag = (spx.Book == BookKind.Acolyte ? "  [A]" : "") + (spx.IsSummon ? "  [summon]" : "");
        ...
        void DrawSaveButtons()
        {
            ...
            {
                _book = CreatureCreator.SharedBook(true);
                _picked = Mathf.Min(_picked, _book.spells.Count - 1);
                _shownFor = null;
            }
        }
        ...
        void DrawPreviewColumn()
        {
            using (new EditorGUILayout.VerticalScope(GUILayout.Width(340)))
            {
                ...
                if (sp != null)
                {
                    int kind = GUILayout.Toolbar(sp.IsSummon ? 1 : 0, new[] { "Spell", "Summon" });
                    if (kind == 0) sp.Creature = "";
                    else if (!sp.IsSummon)
                    {
                        if (_book.creatures.Count > 0) sp.Creature = _book.creatures[0].Name;
                        else ShowNotification(new GUIContent("Make a creature in the Creature Creator first"));
                    }
                }

                var rect = GUILayoutUtility.GetRect(340, 340, GUILayout.ExpandWidth(false));
                var raised = sp != null && sp.IsSummon ? _book.Creature(sp.Creature) : null;
                if (sp != null && sp.IsSummon)
                {
                    // a summon shows the creature it raises
                    if (_shownFor != sp || !_shownSummon || _shownCreature != raised || (raised != null && _shownCreatureBody != raised.Body))
                    {
                        _preview.Show(raised != null ? SpellDef.BodyPrefab(raised.Body) : null);
                        if (raised != null && _preview.Shown != null)
                            CreatureCreator.ShapeOn(_preview.Shown, raised, _preview.Shown.transform.localScale);
                        _shownFor = sp;
                        _shownSummon = true;
                        _shownCreature = raised;
                        _shownCreatureBody = raised != null ? raised.Body : SpellBody.Particle;
                    }
                    if (raised != null)
                        _preview.Tint(raised.PreviewTint(), SpellPayload.StateT01(raised.Payload.State), raised.Skin);
                }
                else if (sp != null)
                {
                    if (_shownFor != sp || _shownSummon)
                    {
                        _shownSummon = false;
                        _shownCreature = null;
                        _preview.Show(SpellDef.BodyPrefab(SpellBody.Particle));
                    ...
                    // a blob is nothing until the numbers colour it
                    var pay = sp.Payload;
                    _preview.Tint(sp.PreviewTint(), SpellPayload.StateT01(pay.State), sp.Skin);
                }
                _preview.Draw(rect, posable: sp != null && !sp.IsSummon);
        ...
        {
            if (sp != null && sp.IsSummon) return; // a summon's shape is its creature's
        ...
        void DrawControls()
        {
            ...
            if (sp.IsSummon)
            {
                // ★ A SUMMON IS CAST, NOT BECOME. Runes raise its creature; the
                // creature itself - born as, can do, shape, look - is the
                // Creature Creator's.
                EditorGUILayout.LabelField("SUMMONS", EditorStyles.boldLabel);
                var creatures = new List<string>();
                foreach (var c in _book.creatures) creatures.Add(c.Name);
                int raises = creatures.IndexOf(sp.Creature);
                if (raises < 0)
                {
                    creatures.Add(sp.Creature + " (not in the book)");
                    raises = creatures.Count - 1;
                }
                int picked = EditorGUILayout.Popup("Creature", raises, creatures.ToArray());
                if (picked < _book.creatures.Count) sp.Creature = _book.creatures[picked].Name;
                if (GUILayout.Button("Open in the Creature Creator")) CreatureCreator.Open(sp.Creature);
        ...
        /// The state material's sliders, shared with the Creature Creator.
        public static void LookSliders(SpellTable.Look k)
        {
        ...
        void DrawRunes(SpellDef sp)
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                var runes = _book.runes; // the twelve and every made rune
                var names = new string[runes.Count + 1];
                names[0] = "add a rune...";
                for (int i = 0; i < runes.Count; i++) names[i + 1] = runes[i].Name;
                int pick = EditorGUILayout.Popup(0, names, GUILayout.Width(140));
                if (pick > 0) sp.Runes.Add(runes[pick - 1].Type);
                if (GUILayout.Button("All twelve", GUILayout.Width(80)))
                { sp.Runes.Clear(); foreach (var r in runes) if (r.BuiltIn) sp.Runes.Add(r.Type); }
                if (GUILayout.Button("Clear", GUILayout.Width(60))) sp.Runes.Clear();
            }
        }

        string RuneName(RuneType r) => _book.Rune(r)?.Name ?? r.ToString();
