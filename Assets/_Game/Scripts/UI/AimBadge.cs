...
namespace SpellyZombie
{
    ...
    public class AimBadge : MonoBehaviour
    {
        ...
        /// The vessel shell is an invisible kinematic copy of a bowl sitting
        /// exactly on the real one, so a ray hits whichever won that frame and
        /// the offer blinks. Grab and ground rays already mask it out.
        public static int AimMask =>
            Physics.DefaultRaycastLayers & ~(1 << VesselShell.Layer);
        ...
        void Resolve(RaycastHit hit)
        {
                ...
                && hit.distance <= ShapeShift.ScanReach)
            {
                ...
                {
                    ScanTarget = scanRoot;
                    // key the badge on the POT, not on whichever of its
                    // colliders the ray won - the debounce must not thrash
                    Point(scanRoot, scanRoot, hit, "F");
                    ScanOfferLive = true;
