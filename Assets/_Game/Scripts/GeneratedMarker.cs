...
{
    /// THE AXIOM'S MARKER (Marko Jul 25: "You have no control, I can change
    ...
    /// violation classes at once. Code must use the component HE authored and
    /// only create one when it is genuinely absent.
    public static class Adopt
    {
        /// Returns the existing component if his prefab already carries one,
        /// otherwise adds it. `created` tells the caller whether it is safe to
        /// configure — never overwrite values he set in the Inspector.
        ...
        /// Same, but searches children too (his art often puts the real
